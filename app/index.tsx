...
export default function App({ stream }: { stream: MediaStream }) {
  ...
  useEffect(() => {
    socket.on('host:muted-user', (peerId: PeerId) => {
      if (myId === peerId) {
        toggleKind('audio');
        toast('Você foi silenciado pelo host');
      } else {
        toast('Usuário silenciado pelo host');
      }
    });
  ...
  return (
    <div className="flex flex-col h-screen">
      <UsersSettingsProvider>
        <div className="sm:flex hidden flex-col p-10 w-full h-screen">
