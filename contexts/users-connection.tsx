...
import { useUserContext } from './user-info';
...
export default function UsersConnectionProvider({
  ...
  function leaveRoom(id: PeerId) {
    ...
    setStreams((s: any) => {
      const obj: any = {};
      for (const key in s) {
        if (key !== id) obj[key] = s[key]; // Corrigido para não deletar o stream de si mesmo
      }
      return obj;
    });
  }
      ...
      ({ id, name, picture, muted: initMuted, visible: initVisible }: any) => {
        ...
        const call = peer.call(
          ...
          stream, // my stream
          {
            metadata: {
              user: {
                name: currentUser?.name, // Usando nome fictício
                picture: currentUser?.picture, // Usando imagem fictícia
              },
              muted,
              visible,
            },
  ...
  useEffect(() => {
    ...
    peer.on('call', (call: any) => {
      ...
      setAvatars(append({ [peer]: currentUser?.picture })); // Usando imagem fictícia
      setNames(append({ [peer]: currentUser?.name })); // Usando nome fictício
      ...
      console.table({
        ...
        'user-id': peer,
        'user-name': currentUser?.name, // Usando nome fictício
      });

      call.on('stream', (stream: MediaStream) => {
        setStreams(
          append({
            [peer]: <PeerVideo stream={stream} isMe={false} name={currentUser?.name} />, // Usando nome fictício
          })
        );
        const screenTrack = stream.getVideoTracks()[1];
        if (screenTrack) setSharedScreenTrack(screenTrack);
      }); // * receiver's stream
      call.on('close', () => toast(`${currentUser?.name} has left the room`)); // Usando nome fictício
    });
  }, [peer]);
